...
// import Eye from "../data/eye-hide.svg"
const InputBox = ({name,type,id,icon,value,dark,placeholder,onChange,disable=false,light}) => {
    const [passVisible,setPassVisible] = useState(false)
        return ( 
        <div className="relative w-[100%] mb-4 mx-auto ">
            {icon? <img className="z-20 w-5 h-5 cursor-pointer absolute top-[50%] translate-y-[-50%] left-3 " src={icon}/>:""}
            <input onChange={onChange} className={"w-full h-full  p-4 relative rounded-xl text-sm placeholder:text-grey dark:text-white placeholder:text-sm dark:placeholder:text-grey text-black   " +(disable?" opacity-80 ":"") +(icon?"pl-11 ":"")+(dark?"dark:bg-main-dark ":"dark:bg-secondary-dark ")+(light=="gray"?"bg-light-gray":"bg-white shadow")}type={type =="password"?passVisible?"text":"password":type} disabled={disable} name={name} defaultValue={value} placeholder={placeholder}  />
            {type=="password"
            ? 
            <svg className={"w-5 h-5 cursor-pointer absolute top-[50%] translate-y-[-50%] right-3 "} onClick={()=>setPassVisible(!passVisible)} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
